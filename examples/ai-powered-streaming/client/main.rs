...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server_url = env::var("SERVER_URL").unwrap_or_else(|_| "http://localhost:8084".to_string());
    let client = A2AClient::new(&server_url)?;

    info!("ai-powered-streaming client → {server_url}");
    ...
    let stream = client
        .stream_message(user_message("Write a haiku about Rust's borrow checker."))
        .await?;
        ...
        match item {
            Ok(event) => {
                ...
                if let Some(artifact) = event.artifact_update {
                    ...
                    info!("[{elapsed:.2}s] #{event_index} → delta: {text:?}{marker}");
                }
            }
            Err(e) => {
