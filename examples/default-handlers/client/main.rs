    ...
    for _ in 0..150 {
        let task = client
            .get_task(GetTaskRequest {
                ...
                tenant: Some("default-handlers".to_string()),
            })
            .await?;
        if matches!(
            task.status.state,
...
fn user_message(text: &str) -> SendMessageRequest {
    SendMessageRequest {
        ...
        metadata: None,
        tenant: "default-handlers".to_string(),
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("default-handlers client → {server_url}");
    ...
    let response = client
        .send_message(user_message(
            "In one sentence, what is the Agent-to-Agent protocol?",
        ))
        .await?;
